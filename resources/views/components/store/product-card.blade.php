...
@php
    ...
    $slug = $product->slug();
    $href = $product->storefrontUrl() ?? route('shop');
    $image = $product->getFirstMedia('images');
    $from = Catalog::fromPrice($product);
@endphp

<a href="{{ $href }}"
   {{ $attributes->merge(['style' => $product->accentStyle()])->class([
    ...
    @if (\App\Support\Catalog::saveUpTo($product) > 0)
        <span class="absolute top-2.5 right-2.5 z-10 rounded-full bg-black/70 px-2 py-0.5 text-[9px] font-extrabold tracking-wider text-[var(--accent)] uppercase ring-1 ring-[var(--accent)]/40 backdrop-blur sm:top-3.5 sm:right-3.5 sm:px-2.5 sm:py-1 sm:text-[10px]">
            Save {{ rtrim(rtrim(number_format(\App\Support\Catalog::saveUpTo($product), 1), '0'), '.') }}%
        </span>
    @endif
    ...
    <div class="relative aspect-[4/5] overflow-hidden bg-accent-electric">
        ...
        @else
            <div class="absolute inset-0 grid place-items-center">
                <span class="display-title text-3xl text-white/15">{{ Str::of((string) $slug)->explode('-')->first() }}</span>
            </div>
        @endif
    </div>

    <div class="flex flex-1 flex-col p-3 sm:p-5">
        <p class="eyebrow text-[9px] text-[var(--accent)] sm:text-[11px]">{{ $product->protocol_label ?? $product->dose }}</p>
