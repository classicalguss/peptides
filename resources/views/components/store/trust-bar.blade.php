<div class="border-y border-white/5 bg-black/60">
    <div class="mx-auto grid max-w-7xl gap-px px-4 sm:grid-cols-2 lg:grid-cols-4">
        @foreach (site_list('global.trust') as $item)
            <div class="flex items-start gap-3 py-6 sm:px-4">
                ...
                <div>
                    <p class="text-[13px] font-extrabold tracking-wide text-white uppercase">{{ $item->heading }}</p>
                    <p class="mt-0.5 text-xs text-white/45">{{ $item->body }}</p>
                </div>
            </div>
        @endforeach
    </div>
